...
6. **`Commands\GenerateTypesCommand`** (`typed-i18n:generate`) resolves a `TranslationSource` from the container, reads keys for the reference locale, writes the `.ts`, then reports drift. Options `--locale`, `--output`. When the source is a `LaravelLangSource` whose `langPath` is missing, it fails (exit 1).
